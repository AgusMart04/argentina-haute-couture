  ...
  return (
    <section className="py-28 md:py-40 px-6 md:px-12 bg-ivory">
      <div className="mx-auto max-w-[1500px]">
        <Reveal><p className="label-eyebrow mb-6">09 · Hoja de ruta</p></Reveal>
        <Reveal delay={100}>
          ...
          <div className="hidden md:grid grid-cols-6 gap-6">
            {steps.map((step, i) => (
              <Reveal key={step.p} delay={i * 100} className="relative">
                <div className={`h-6 w-6 rounded-full mb-6 -mt-1.5 border-4 border-ivory ${step.final ? "bg-sol shadow-[0_0_30px_var(--color-sol)]" : "bg-celeste-deep"}`} />
                <p className={`font-display text-2xl ${step.final ? "text-tango" : "text-ink"}`}>{step.p}</p>
                ...
                <Reveal key={step.p} delay={i * 100} className="relative flex items-start">
                  {isLeft ? (
                    <>
                      ...
                      <div className={`absolute left-1/2 -translate-x-1/2 top-1 h-6 w-6 rounded-full border-4 border-ivory z-10 ${step.final ? "bg-sol shadow-[0_0_30px_var(--color-sol)]" : "bg-celeste-deep"}`} />
                      <div className="w-1/2" />
                    </>
                  ) : (
                    <>
