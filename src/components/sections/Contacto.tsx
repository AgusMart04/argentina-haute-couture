...
export default function Contacto() {
  ...
  return (
    <section
      ...
      className="py-28 md:py-40 px-6 md:px-12 bg-ivory"
    >
      {submitted ? (
        <div className="mx-auto max-w-[800px] text-center">
          <Reveal>
            <div className="inline-flex items-center justify-center h-20 w-20 rounded-full bg-celeste/15 mb-8">
              <svg
                className="h-10 w-10 text-celeste-deep"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
          ...
          <Reveal delay={200}>
            <p className="mt-8 text-ink-soft font-light text-xl max-w-lg mx-auto">
              Gracias, {formData.name}. Te contactaremos pronto en{" "}
          ...
          <Reveal delay={300}>
            <button
              onClick={() => {
                setSubmitted(false);
                setFormData({ name: "", company: "", role: "", email: "", mensaje: "" });
                setSelectedTier("Patrocinador");
                setErrors({});
              }}
        ...
        <div className="mx-auto max-w-[1500px] grid lg:grid-cols-12 gap-16">
          <div className="lg:col-span-5">
            <Reveal>
              <p className="label-eyebrow mb-6">10 · Contacto</p>
            </Reveal>
            <Reveal delay={100}>
              <h2 className="font-display text-5xl md:text-6xl leading-[0.95] text-ink text-balance">
          ...
          <Reveal delay={150} className="lg:col-span-7">
            ...
            >
              <label className="bg-ivory p-6 block group">
                <span className="label-eyebrow text-ink-soft text-xs group-focus-within:text-celeste-deep transition">
                  Nombre *
                </span>
                <input
                  type="text"
              ...
              <label className="bg-ivory p-6 block md:col-span-2 group">
                ...
                <div ref={tierRef} className="relative mt-3">
                  ...
                  <svg
                    className={`pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 h-4 w-4 text-celeste-deep transition-transform duration-200 ${tierOpen ? "rotate-180" : ""}`}
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                  ...
                  <div
                    className={`absolute z-50 mt-2 w-full bg-ivory border border-border rounded-sm shadow-lg origin-top transition-all duration-150 ${tierOpen ? "opacity-100 scale-y-100" : "opacity-0 scale-y-0 pointer-events-none"}`}
                  >
                    {["Patrocinador", "Inversor", "Diseñador", "Marca", "Prensa", "Alianza"].map(
                      (o) => (
                        <button
                          key={o}
                          type="button"
              ...
              <div className="md:col-span-2 bg-ivory p-6 flex justify-end">
                <button
                  type="submit"
                  className="group relative overflow-hidden gradient-argentina animate-gradient-shift text-ivory px-12 py-4 label-eyebrow hover:scale-105 hover:shadow-[0_20px_50px_-10px_rgba(30,111,184,0.8)] transition-all duration-300 shadow-[0_15px_40px_-15px_rgba(30,111,184,0.6)]"
                >
